...
void mexFunction(int nlhs, mxArray *plhs[],
  ...
  int topK = mxGetScalar(prhs[2]);  
  
  plhs[0] = mxCreateDoubleMatrix(topK,1,mxREAL); /* create the output vector */
  ...
  {   
     ...
     // maxIndex[j] = min(topK-1, maxIndex[j]); // C indexing from 0 to topK-1
     if(maxIndex[j] > topK-1) maxIndex[j] = topK-1;
  }
